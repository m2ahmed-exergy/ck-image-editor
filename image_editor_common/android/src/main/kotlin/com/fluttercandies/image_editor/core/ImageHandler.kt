...
//    canvas.drawText(text.text, text.x.toFloat(), text.y.toFloat(), textPaint)
    ...
    private fun getStaticLayout(text: Text, textPaint: TextPaint, width: Int): StaticLayout {
        var layoutTextAlign = when(text.textAlign){
            Paint.Align.CENTER -> {
        ...
        return if (Build.VERSION.SDK_INT >= 23) {
            ...
            ).setAlignment(layoutTextAlign).setLineSpacing(0.0F, 1.0F).setIncludePad(true).build();
        } else {
            StaticLayout(text.text, textPaint, width, layoutTextAlign, 1.0F, 0.0F, true)
        }
    }
